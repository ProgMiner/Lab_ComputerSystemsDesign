...
#define LCD_MAX_BYTES_PER_STEP 40
...
void lcd_init(I2C_HandleTypeDef * i2c) {
	...
	lcd_reset_screen();
	HAL_I2C_Mem_Write(i2c, LCD_I2C_WRITE_ADDRESS, LCD_I2C_DATA_CONTROL_BYTE, 1, buf, sizeof(buf), sizeof(buf) * 10);
